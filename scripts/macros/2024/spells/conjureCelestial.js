...
async function move({trigger: {castData}, workflow}) {
    ...
    let position = await crosshairUtils.aimCrosshair({token: workflow.token, maxRange: 30, centerpoint: templatePosition, crosshairsConfig: {icon: effect.img, resolution: 2, size: templateUtils.getTemplateDistance(template)}, drawBoundries: true});
