...
async function use({trigger, workflow}) {
    ...
    let templateRay = templateUtils.getTemplateRay(workflow.template);
    await Promise.all(workflow.failedSaves.map(async token => {
        let ray = foundry.canvas.geometry.Ray.fromAngle(token.x, token.y, templateRay.angle, templateRay.distance);
        await tokenUtils.moveTokenAlongRay(token, ray, 30);
        await effectUtils.applyConditions(token.actor, ['prone']);
    }));
    let ray = foundry.canvas.geometry.Ray.fromAngle(workflow.token.x, workflow.token.y, templateRay.angle, templateRay.distance);
