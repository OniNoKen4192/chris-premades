...
async function reaction({trigger: {entity: item, target}}) {
    ...
    let change = effectUtils.getChanges(target).find(i => i.key === 'flags.midi-qol.actions.reactionsUsed');
    if (!change) return;
...
async function updated({trigger: {entity, item}}) {
    ...
    if (item.system.uses.value) {
        if (effectUtils.getChanges(effect)[0].value != '2') await genericUtils.update(effect, effectUtils.changesUpdateData([{key: 'flags.midi-qol.actions.reactionsMax', mode: 5, priority: 20, value: '2'}]));
    } else {
        if (effectUtils.getChanges(effect)[0].value != '1') await genericUtils.update(effect, effectUtils.changesUpdateData([{key: 'flags.midi-qol.actions.reactionsMax', mode: 5, priority: 20, value: '1'}]));
    }
}
async function turn({trigger: {entity: item}}) {
    ...
    let change = effectUtils.getChanges(effectData).find(i => i.key === 'flags.midi-qol.actions.reactionsUsed');
    if (change) change.value = '1';
    await genericUtils.update(reactionUsed, effectUtils.changesUpdateData(effectUtils.getChanges(effectData)));
}
