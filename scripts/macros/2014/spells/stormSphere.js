...
async function late({workflow}) {
    ...
    if (!template) return;
    let targetToken = workflow.targets.first();
    new Sequence().effect().atLocation(templateUtils.getTemplatePosition(template)).stretchTo(targetToken).file('jb2a.chain_lightning.primary.blue').missed(!workflow.hitTargets.has(targetToken)).play();
}
