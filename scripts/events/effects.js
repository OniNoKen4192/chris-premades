...
async function createActiveEffect(effect, options, userId) {
    ...
    if (effect.parent instanceof Actor && effectUtils.getChanges(effect).find(change => change.key.includes('system.attributes.movement.'))) await effects.specialDurationZeroSpeed(effect.parent);
}
