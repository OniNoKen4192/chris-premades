...
async function aimCrosshair({token, maxRange, crosshairsConfig, centerpoint, drawBoundries, customCallbacks, trackDistance = true, fudgeDistance = 0, validityFunctions = []}) {
    ...
    let checkDistance = async (crosshairs) => {
        if (maxRange && drawBoundries) {
            ...
            let matchTemplates = game.settings.settings.has('core.gridTemplates') && game.settings.settings.has('core.gridDiagonals') && game.settings.get('core', 'gridTemplates') && (game.settings.get('core', 'gridDiagonals') !== CONST.GRID_DIAGONALS.EXACT);
            if (matchTemplates) {
